...
function validateForm() {
    const searchInput = document.getElementById('kyw');
    if (searchInput.value.trim() === '') {
        searchInput.placeholder = 'Vui lòng nhập thông tin tìm kiếm';
...
function resetPlaceholder() {
    const kyw = document.getElementById('kyw');
    kyw.placeholder = 'Search For items...'; // Khôi phục placeholder ban đầu
    kyw.classList.remove('error'); // Bỏ lớp CSS lỗi (nếu có)
}
$(document).ready(function () {
    // Nút cộng
    $('#add').on('click', function () {
        var currentQuantity = parseInt($('#quantityDisplay').val());  // Lấy số lượng hiện tại
        if (!isNaN(currentQuantity)) {
            $('#quantityDisplay').val(currentQuantity + 0);  // Tăng số lượng lên 1
            $('#quantityInput').val(currentQuantity + 0);  // Cập nhật giá trị của input ẩn
        }
    });
    ...
    $('#sub').on('click', function () {
        ...
        if (!isNaN(currentQuantity) && currentQuantity > 1) {
            $('#quantityDisplay').val(currentQuantity - 0);  // Giảm số lượng xuống 1
            $('#quantityInput').val(currentQuantity - 0);  // Cập nhật giá trị của input ẩn
        }
    });
});
document.querySelectorAll('.plus, .minus').forEach(button => {
    button.addEventListener('click', function() {
        ...
        const input = document.querySelector(`input[name="quantityDisplay[${key}]"]`);
        let quantity = parseInt(input.value);

        const price = parseFloat(document.querySelector(`tr:nth-child(${key + 1}) td.cr-cart-price span.amount`).textContent.replace('$', ''));
        ...
        const newSubtotal = Math.floor(quantity * price); 
        document.getElementById(`subtotal-${key}`).textContent = `$${newSubtotal}`;
    });
});
...
function incrementViews(idproduct) {
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "increment_views.php?idproduct=" + idproduct, true);
    xhr.send();
}
